...
//                $discount = $cartObj->discount;
                ...
                if($cartObj->promo_code != null && $cartObj->promo_code != '') {
                    //get promo code detail
                    $promoCodeArr = PromoCodes::getPromoCodeDetail($cartObj->promo_code);
                    
                    //Apply till maximum allowed discount only
                    $discount = ($total / 100) * $promoCodeArr['discount_percentage'];
                    if(intval($promoCodeArr['max_dis_amt']) != 0 && $promoCodeArr['max_dis_amt'] < $discount) {
                        $discount = $promoCodeArr['max_dis_amt'];
                    }
                    $cartObj->discount = round($discount, 2);
                }
            }
...
//    public function updateDiscountIfAny($cart_id) {
//        DB::beginTransaction();
//        try {
//            $cart = Cart::where(['id' => $cart_id])->first();
//            //get promo code detail
//            $promoCodeArr = PromoCodes::getPromoCodeDetail($cart->promo_code);
//            
//            //Apply till maximum allowed discount only
